...
projection(lw) <-projection("+proj=longlat") 
spTransform(lw, CRS("+proj=longlat"))
Step1toplot<-extract(lst,lw,buffer=1000,fun=mean,na.rm=T) %>%
  t() %>%
  as.data.frame() %>%
  mutate(date = getZ(lst))
ANSWERTOSTEP1<-ggplot(Step1toplot, aes(x=date, y = V1)) + 
  geom_point() + 
  geom_smooth(span = .009, n = 100) + 
...
ANSWERTOSTEP2<-gplot(lst_month) + geom_tile(aes(fill = value)) +
  facet_wrap(~ variable) +
